...
int main(int argc, char** argv)
{
  ...
  // Create a publisher that can publish joint 1 position commmands
  ros::Publisher joint1_pub =
    n.advertise<std_msgs::Float64>("/simple_arm/joint_1_position_controller/command", 10);
  
  // Create a publisher that can publish joint 2 position commmands
  ...
  // Get ROS start time
  while (not start_time) // start_time will be 0 until first message received on clock topic
  {
