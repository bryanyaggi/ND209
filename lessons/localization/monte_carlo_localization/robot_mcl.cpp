...
int main()
{
  ...
  Robot particles[numParticles];
  for (int i = 0; i < numParticles; i++)
  {
    particles[i].set_noise(.05, .05, 5.0);
  }

  for (int t = 0; t < numSteps; t++)
  {
    // Move robot, then sense environment
    myrobot.move(turn, forward);
    z = myrobot.sense();
    
    // Simulate robot motion for each particle
    Robot movedParticles[numParticles];
    for (int i = 0; i < numParticles; i++)
    {
      movedParticles[i] = particles[i].move(turn, forward);
      particles[i] = movedParticles[i];
    }

    // Generate particle weights based on robot's measurement
    double w[numParticles];
    for (int i = 0; i < numParticles; i++)
    {
    ...
    // Resample
    Robot newParticles[numParticles];
    int index = round(gen_real_random() * numParticles);
    double beta = 0.0;
    double maxW = max(w, numParticles);
    for (int i = 0; i < numParticles; i++)
    {
      beta += gen_real_random() * 2.0 * maxW;
      while (beta > w[index])
      {
        beta -= w[index];
    ...
    // Evaluate error
    double error = evaluation(myrobot, particles, numParticles);
    cout << "step " << t << ": average error = " << error << endl;

    // Graph
    visualization(numParticles, myrobot, t, movedParticles, newParticles);
  }
